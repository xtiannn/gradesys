        ...
        try {
            ...
            while ($sheet->getCell('A' . $row)->getValue() != '') { // Check if row is not empty
                ...
                if($deptID == 3){
                    ...
                    $subjectID = $sheet->getCell('K' . $row)->getValue(); 
                ...
                $valsubjectID = isset($_POST['valsubjectID']) ? $_POST['valsubjectID'] : null; //from hidden input in form
                $faID = isset($_POST['faID']) ? $_POST['faID'] : null; //from hidden input in form



                if (empty($lrn) || empty($studname)) {
                    header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&semID=$semID&deptID=$deptID&programID=$valprogramID&excstatus=invalid");

                    exit;
                }
                if($deptID == 3){
                    if (($semID != $valsemID) || ($secID != $valsecID) || ($ayName != $valayName) || ($subjectID != $valsubjectID)) {
                        header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&semID=$semID&deptID=$deptID&programID=$valprogramID&excstatus=notMatched");
                        exit();
                    }
                }else{
                    if (($secID != $valsecID) || ($ayName != $valayName) || ($subjectID != $valsubjectID)) {
                        header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&deptID=$deptID&programID=$programID&excstatus=notMatched");
                        exit();
            ...
            if($deptID == 3){
                header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&semID=$semID&deptID=$deptID&programID=$valprogramID&excstatus=failed&msg=" . $e->getMessage());


            }else{
                header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&deptID=$deptID&programID=$programID&excstatus=failed&msg=" . $e->getMessage());
   
            }
        }
            if ($success) {
                if($deptID == 3){
                    header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&semID=$semID&deptID=$deptID&programID=$valprogramID&excstatus=success");
                }else{
                    header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&deptID=$deptID&programID=$programID&excstatus=success");
                }
            }
    } else {
        if($deptID == 3){
            header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&semID=$semID&deptID=$deptID&programID=$valprogramID&excstatus=noFile");
        }else{
            header("Location: ../students.php?subjectID=$subjectID&facultyID=$facultyID&faID=$faID&secID=$secID&deptID=$deptID&programID=$programID&excstatus=noFile");
        }
    }
}
?>
