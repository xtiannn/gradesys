                ...
                <form action="save_curriculum.php" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                    <fieldset class="border p-4 rounded mb-4">
                                    ...
                                    if(isset($_GET['program_id'])) {
                                        ...
                                        try {
                                            ...
                                            WHERE programID = :programID 
                                              AND subjectID IN (SELECT subjectID 
                                                                 FROM curriculum 
                                                                 WHERE gradelvlID IS NULL AND semID IS NULL)";                                    
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <?php
                                    try {
                                        $sql = "SELECT * FROM grade_level WHERE isActive=1";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->execute();
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="prere" class="form-label fw-bold">Prerequisites:</label>
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="bi bi-shield-check"></i></span>
                                    </div>
                                    <select class="selectpicker form-control" multiple data-live-search="true" id="prere" name="selPre[]" required>
                                        <option value="0" selected>No Prerequisites</option>
                                        <?php
                                        include("../includes/config.php");
                                        try {
                                            $sql = "SELECT c.*, (SELECT s.subjectname FROM subjects s WHERE s.subjectID = c.subjectID) AS subjectName,
                                                    ...
                                                    AND c.subjectID IN 
                                                        (SELECT subjectID FROM subjects WHERE programID = :programID)";
                                            $stmt = $conn->prepare($sql);
                                            $stmt->bindParam(':programID', $programID, PDO::PARAM_INT);
                                            $stmt->execute();
            ...
            <div class="modal-body">
                <form action="update_curriculum.php" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                    <fieldset class="border p-4 rounded mb-4">
                        ...
                        <div class="row mb-3">
                        <div class="col-md-12 mb-3">
                            <label for="upselsub" class="form-label fw-bold">Select Subject:</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                <span class="input-group-text"><i class="bi bi-journal"></i></span>
                                </div>
                                <select class="form-select form-control selectpicker" id="upselsub" name="selSub" required>
                                <option selected disabled value="">Select Subject</option>
                                    <?php
                                    include("../includes/config.php");
                                    ...
                                    if(isset($_GET['program_id'])) {
                                        ...
                                        try {
                                            $sql = "SELECT * FROM subjects WHERE programID = :programID";                                           
                                            $stmt = $conn->prepare($sql);
                                            $stmt->bindParam(':programID', $programID, PDO::PARAM_INT);
                                            $stmt->execute();
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="prere2" class="form-label fw-bold">Prerequisites:</label>
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="bi bi-shield-check"></i></span>
                                    </div>
                                    <select class="selectpicker form-control" multiple data-live-search="true" id="prere2" name="selPre[]" required>
                                        <option value="0">No Prerequisites</option>
                                        <?php
                                        include("../includes/config.php");
...
$(document).ready(function() { 
    ...
    var prereq = $(this).data('curri-prereq');
    ...
    $('#upselsub option, #upseltype option, #uplevel option, #upterm option, #prere2 option').prop('selected', false);

    // Split the prereq string into an array
    var prereqArray = prereq.split(', ');

    // Iterate over each select and set the appropriate options to selected
    $('#upselsub option').each(function() {
        if ($(this).val() == subjectID) {
            $(this).prop('selected', true);
        }
    });

    $('#upseltype option').each(function() {
        if ($(this).val() == typeID) {
            $(this).prop('selected', true);
        }
    });

    $('#uplevel option').each(function() {
        if ($(this).val() == levelID) {
            $(this).prop('selected', true);
        }
    });

    $('#upterm option').each(function() {
        if ($(this).val() == semID) {
            $(this).prop('selected', true);
        }
    });
    ...
    $('#prere2 option').each(function() {
        if (prereqArray.includes($(this).val())) {
            $(this).prop('selected', true);
        }
    });

    $('#upselsub, #upseltype, #uplevel, #upterm, #prere2').trigger('change');

    $('#updateCurr').modal('show');
    });

    $('#prere').on('change', function() {
        var selectedValues = $(this).val();
        if (selectedValues && selectedValues.includes('0') && selectedValues.length > 1) {
            // If 'None' (value '0') is selected alongside other options, deselect it
            var index = selectedValues.indexOf('0');
            selectedValues.splice(index, 1);
            $(this).val(selectedValues);
            $('.selectpicker').selectpicker('refresh'); // Refresh the selectpicker
        }
    });
    $('#prere2').on('change', function() {
        var selectedValues2 = $(this).val();
        if (selectedValues2 && selectedValues2.includes('0') && selectedValues2.length > 1) {
            // If 'None' (value '0') is selected alongside other options, deselect it
            var index2 = selectedValues2.indexOf('0');
            selectedValues2.splice(index2, 1);
            $(this).val(selectedValues2);
            $('.selectpicker').selectpicker('refresh'); // Refresh the selectpicker
        }
    });
});
