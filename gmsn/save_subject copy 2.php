...
if (isset($_POST['saveSubjectBtn'])) {
    ...
    $code = $_POST['txtSubjectCode'] ?? null;
    $desc = isset($_POST['txtSubjectName']) ? ucwords(strtolower($_POST['txtSubjectName'])) : null;
    $progs = $_POST['selProg'] ?? null;
    $subjectID = $_POST['subjectID'];
    $status = $_POST['status'] ?? null;

    if (!empty($status) && (empty($code) && empty($desc) && empty($progs))) {
        // Case 1: Update only the isActive status
        try {
            $query = "UPDATE subjects SET isActive = ? WHERE subjectID = ?";
            $stmt = $conn->prepare($query);
            $stmt->execute([$status, $subjectID]);
    ...
    } elseif (!empty($code) && !empty($desc) && !empty($progs)) {
        // Case 2: Update subject details and associated programs/curriculum
        try {
            $conn->beginTransaction();
        ...
        $response['status'] = 'error';
        $response['message'] = "Invalid input. Please ensure all required fields are filled.";
    }
}
